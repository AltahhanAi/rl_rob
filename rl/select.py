...
class Runs: # experimental trials
    ...
    def init(self):
        np.random.seed(self.seed)# for binomial, choice and randint
        random.seed(self.seed)   # for using choices
    ...
    def interact(self, label='', frmt='-', **kw):
        ...
        for self.run in range(runs):
            ...
            algorithm = algorithm.interact(visual=visual, label=label_, seed=self.seed+run, **kw)
    ...
    def _plot_mean_std(self, x, Y, frmt='-', label=''):
        mean = Y.mean(axis=0)
        line, = plt.plot(x, mean, frmt, label=label)
        if self.fill:
            std  = Y.std(axis=0)
            deviate = 1.96 * std / np.sqrt(self.runs) if self.ci else std
            plt.fill_between(x, mean - deviate, mean + deviate, alpha=0.2, color=line.get_color())
    ...
    def plot(self, label='', frmt='-') :
        ...
        if self.plotT: self._plot_mean_std(algorithm.eplist, Ts, frmt, label); plt.xlabel('episodes,'+label_); plt.legend()
        if self.plotR: self._plot_mean_std(algorithm.eplist, Rs, frmt, label); plt.xlabel('episodes,'+label_); plt.legend()
        if self.plotE: self._plot_mean_std(algorithm.eplist, Es, frmt, label); plt.xlabel('episodes,'+label_); plt.legend()

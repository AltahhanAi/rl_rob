...
'''
   This is a full-fledged RL library. The assumption is a tabular representation.
    ...
    We begin by defining a base class called MRP (Markov Reward Process). This class focuses
    ...
    on control by implementing policy changes/improvement capabilities. It will allow us to change the 
    Q table, which indirectly influences the probabilities of taking specific actions. The Q table specifies
    the usefulness (expected discounted sum of rewards) of taking a specific action in a certain state.
...
class MRP:
    ...
    def __init__(self, env=randwalk(), γ=1, α=.1, dα=1, v0=0, randv0=True, episodes=100, view=1, 
        ...
        self.policy = self.stationary # the default policy is a stationary one where actions do not change
    ...
    # set up the V table
    def init_(self):
        self.V = np.random.rand(self.env.nS) if self.randv0 else np.ones(self.env.nS)
        self.V *=self.v0
    ...
    # accommodates Sarsa style algorithms
...
class MRP(MRP):
    ...
    def __init__(self, plotV=False,  plotT=False, plotR=False, plotE=False, ylim=None, animate=False, **kw):
        super().__init__(**kw)
    ...
    def plot_ep(self, animate=None, plot_exp=False, label=''): 
        ...
        for plot, ydata, label_ in zip([self.plotT, self.plotR, self.plotE], 
            ...
            plt.xlabel('episodes')
            plt.legend()
            if self.ylim is not None and not self.plotE: plt.ylim(0,self.ylim)
            i+=1
        ...
        if self.plotV or self.plotE or self.plotT or self.plotR:
            figsizes = list(zip(plt.gcf().get_size_inches(), getattr(self.env, "figsize0", (12, 2))))
...
class MRP(MRP):
    ...
    def stop_(self,done): # terminated or truncated
        if done:
            self.skipstep +=1                       # holds the count for how many steps after ep is finished (will reach n-1)
            if self.skipstep == self.n:             # The below is executed before self.t+=1 in interact() and hence +1 is necessary
                self.t = self.t+1 - self.skipstep   # returns t to its original actual count of number of steps.              
                self.skipstep = 0
                return True
            return False
        self.skipstep = 0
    ...
    def G(self, τ1, τn):    # n-steps return, called during an episode
        #if self.γ==1: return self.r[τ1:τn+1].sum() # this saves computation when no discount is applied
        Gn = 0
...
def MDP(MRP=MRP):
    class MDP(MRP):
        ...
        def π(self, sn,  a=None):
            ...
            π = Qsn*0 + ε/nA
            π[Qsn.argmax()] += 1-ε
            return π if a is None else π[a]

        # returns whether the current policy is optimal by checking if the agent can reach the goal in self.Tstar
        def πisoptimal(self):
            s = self.env.reset()
            done = False
            for t in range(self.Tstar):
                s,_, done,_,_ = self.env.step(self.greedy_(s))
            return done
...
'''
    all *discrete-action policy-gradient control algorithms* must inherit this class

'''
def PG_disc(MDP=MDP(MRP)):
    class PG_disc(MDP):
        ...
        def τsoftmax(self, s):
            ...
            # sample from the policy probabilities
            a = choices(range(self.env.nA), weights=π, k=1)[0]
            self.isamax = a in np.where(π==π.max())[0]
            return a

        # overriding π() in parent class MDP: 
        # in MDP π() returns probabilities according to an εgreedy
...
def PG_cont(MDP=MDP(MRP)):
    class PG_cont(MDP):
        ...
        #------------------------------------- continuous policy 🧠------------------------------------
        # returns a softmax continuous action
        def τsoftmax(self, s):
            if self.dτ < 1: self.τ = max(self.τmin, self.τ*self.dτ)              # exponential decay
            if self.Tτ > 0: self.τ = max(self.τmin, self.τ0 - self.t_ / self.Tτ) # linear      decay
        ...
        # overriding π() in parent class MDP: 
        # in MDP      π() returns probabilities according to an εgreedy
        # in PG_disc  π() returns probabilities according to a τsoftmax
        # in PG_cont  π() returns actual action according to a τsoftmax

        # continuous action can have multiple components, each of which has a continuous value
        def π(self, s):
            Qs = self.Q_(s)
            exp = np.exp(Qs*self.τ)
            return exp/exp.sum()
...
def demo(what='V'):
    switch = {
        'E':    {'plotE':True, 'plotV':True, 'animate':True},                    # suitable for prediction
        'Q':    {'plotT':True, 'visual':True, 'underhood':'maxQ'},               # suitable for control, only for Grid stuff
        'T':    {'plotT':True, 'visual':True},                                   # suitable for control
        'R':    {'plotR':True, 'visual':True},                                   # suitable for control

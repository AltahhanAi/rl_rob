...
'''
    ...
    on control by implementing policy changes/improvements in capabilities. It will allow us to change the 
    ...
    learns the best policy instead of indirectly learning a Q function that, in turn, influences the policy.
...
class MRP:
    ...
    def interact(self, train=True, resume=False, save_every=None, save_final=None,
        ...
        try:
            ...
            while not self.stop_exp():
                ...
                # initial step
                s,a = self.step_0()
                s,_,_,a,_,_ = self.type_convert(s,None,None, a, None, None)
    ...
    # returns indexes of last n elements that span two edges of an array, i is the current index
    # also it returns the element of the current index
        ...
        if done:
            ...
            if self.skipstep == self.n: 
                self.t = self.t+1 - self.skipstep  # returns t to its original actual count of number of steps.  
                                                   # it is executed before self.t+=1 in interact, and hence +1 is necessary
                self.skipstep = 0
                return True
            return False
        self.skipstep = 0
...
def MDP(MRP=MRP):
    class MDP(MRP):
        ...
        # returns a pure one greedy action, **not to be used in learning**
        def greedy_(self, s):
        ...
        def greedy(self, s): 
            ...
            # argmaxes, is defined in dp module, gives multiple argmaxes and is based also on ≈ not = (Qs[i]≈Qs[j])
            # choices() is more efficient than choice()
            return choices(np.argmaxes(Qs))[0]     #return one of the argmaxes actions randomly
        ...
        def π(self, s,  a=None):
            ...
            π = Qs*0 + ε/nA; As = np.argmaxes(Qs)  # returns multiple argmaxes if exist
            π[As] += (1-ε)/len(As)
        ...
        def render(self, rn=None, label='', **kw):
            if rn is None: rn=self.rn
            param = {}
            if 'Q' in self.underhood: param = {'Q':self.Q_()}  # 'maxQ' or 'Q'
            if 'π' in self.underhood: param = {'π':self.Q_()}  # 'π'
...
demoπ =    {'plotT':True, 'visual':True, 'underhood':'π'}                  # suitable for control, only for Grid stuff

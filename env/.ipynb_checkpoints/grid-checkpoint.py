...
def Maze(Grid=Grid, sparse=False, size='large', **kw):
    div = {'large':1, 'mid':2, 'small':3}
    rows, cols = 30//div[size], 30//div[size]
    return Grid(gridsize=[rows, cols], style='maze', reward='reward_' if sparse else 'reward_1', s0=(rows//2)*cols, goals=[rows*cols-1], figsize=[cols,2.5], **kw)

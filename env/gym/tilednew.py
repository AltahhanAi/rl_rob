...
class TileCoder:
    def __init__(self, X0=None, Xn=None, n_tiles=8, n_tilings=1, low=None, high=None, **kw):
        ...
        self.shape     = tuple([self.n_tilings] + [self.n_tiles[i] + p for i, p in enumerate(self.primes)])  # (n_tilings, n_tiles+2, n_tiles+3,...)
        self.nF        = int(np.prod(self.shape))
        self._φ        = np.zeros(self.nF, dtype=np.float32)                                # preallocated flat feature vector — no alloc per step
    ...
    def s_(self, x):
        self._φ[:] = 0                                                                       # zero in place — no allocation
        φ = self._φ.reshape(self.shape)                                                      # free view, no copy
        for ind in self.inds(x): φ[ind] = 1
        return self._φ                                                                       # already flat — no flatten() needed
...
class HashedTileCoder(TileCoder):
    def __init__(self, nF=1024, hash_size=None, **kw):
        ...
        self._φ = np.zeros(self.nF, dtype=np.float32)                                       # reallocate after nF override
...
class IHTTileCoder(TileCoder):
    ...
    def s_(self, x):
        self._φ[:] = 0                                                                       # zero in place — no allocation
        inds = np.where(super().s_(x) != 0)[0]                                              # get exact indices from parent
...
GymTiledExact  = GymTiledFactory(TileCoder)         # exact — zero collision, nF grows with n_tilings × ∏(n_tiles+p)

...
# ========================================= Tile Coder Class ==========================================
class TileCoder:
    def __init__(self, X0, Xn, n_tiles=8, n_tilings=1, **kw):
        self.X0        = np.asarray(X0)
        self.Xn        = np.asarray(Xn)
        self.n_tiles   = n_tiles
        self.n_tilings = n_tilings
        self.primes    = primes(len(self.X0))                                                # [2,3,5,...] ensures tilings never overlap
        self.shape     = tuple([self.n_tilings] + [self.n_tiles + p for p in self.primes])  # mirrors (n_tilings, n_tiles+2, n_tiles+3)
    ...
    def inds(self, x):
        ...
        s_all    = (s_tiling + primes*tilings) // n_tilings
...
class HashedTileCoder(TileCoder):
    def __init__(self, nF=1024, **kw):
        super().__init__(**kw)
        self.nF = nF            # fixed size, overrides exact size
...
class IHTTileCoder(TileCoder):
    def __init__(self, nF=1024, **kw):
        super().__init__(**kw)
        self.nF = nF        # fold exact indices into smaller table

...
class Grid:
    ...
    def reset(self, withtrace=True, **kw): # **kw for passing the seeds to gym 
        ...
        return self.s_()
    #-----------------------------------------rewards related-------------------------------------------
    # This preserves the order implied by: [intermediate, goal1, goal2, cliff] while removing duplicates.
    def rewards_set(self):
        return np.array(list(dict.fromkeys(self.rewards)), dtype=float) # better than return np.array(list(set(self.rewards)))

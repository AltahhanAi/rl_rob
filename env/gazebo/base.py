'''
   Author: Abdulrahman Altahhan,  2026.
   version: 3.6

    This library of functionality in RL aims for simplicity and general
...
model_path = "/opt/ros/foxy/share/turtlebot3_gazebo/worlds/turtlebot3_stage4/burger.model"


def name(): return 'node'+str(randint(1, 1000))


def load_gazebo(env_name='turtlebot3_worlds'):
    subprocess.Popen(["ros2", "launch", "turtlebot3_gazebo", env_name+".launch.py"])
...
def set_nscans_LiDAR(nscans, path=sdf_path):
    ...
    # warn the user that they need to restart gazebo
    print('You will need to re-launch your gazebo environment for the changes to take effect ')
...
class RobEnv(Node):
    # initialisation--------------------------------------------------------------
    # n sets how frequent the publisher is publishing to the /cmd_vel
    def __init__(self, name=name(),
        ...
        θresol = 2*pi/nturns
        dims = [4, 4]
        self.xdim = dims[0]  # related to the size of the environment
        self.ydim = dims[1]  # related to the size of the environment
...
# ---------------------------------------------low level sensing---------------------------------------------
    ...
    def scan(self, scans):
        self.scans = np.array(scans.ranges, dtype=float)

        # invalid readings are treated as no nearby obstacle
        self.scans[np.isnan(self.scans)] = self.max_range
        self.scans[np.isinf(self.scans)] = self.max_range
...
# ---------------------------------------------low level control---------------------------------------------
    ...
    def step(self, a=1, speed=None, θspeed=None):
        ...
        # move and stop so that we can have a well defined action
    ...
    # reseting--------------------------------------------------------------
    def reset(self):
        # stop first to flush any earlier queued motion
        self.robot.linear.x = .0
        self.robot.angular.z = .0
        self.spin_n(self.n)
        ...
        # allow callbacks to refresh odom and scan after reset
        ...
        self.goal_dist, goal = self.nearest_('distgoal')
        self.θgoal_dist = self.θdistgoal(goal)
...
# -----------------------------------learning related (reward and state)---------------------------------------------
    ...
    # generic: returns the goal with the nearest orientation or nearest distance........
    def nearest_(self, compare='distgoal'):
        '''
        passing 'θdistgoal' returns the angular distance of the goal from the nearest orientation
        passing 'distgoal' returns the Euclidean distance of the goal from the nearest distance
        '''
    ...
    # returns the Euclidean distance to the nearest goal
    def nearest(self):
        goal_dist, goal = self.nearest_('distgoal')
        θgoal_dist      = self.θdistgoal(goal)
        return goal_dist, θgoal_dist

    # returns angular distance to the goal in sight
    def θnearest(self, normalised=False):
        θgoal_dist, goal = self.nearest_('θdistgoal')
        goal_dist        = self.distgoal(goal)
        return goal_dist, θgoal_dist


    # ϕ_i= 2πi/k: pool LiDAR beams into k angular sectors and return the nearest normalised obstacle per sector
    def φnearest(self, k=12, normalise=False):
        # divide the scans into k sectors, each with n adjacent scans, and take the min of each
        n = len(self.scans)//k
        φ = self.scans[:k*step].reshape(k, n).min(axis=1)
        return φ
    ...
    def goal_seeking(self):
        ...
        self.at_goal = self.atgoal(self.goal_dist)

        return self.at_goal or self.at_wall

    # set reward based on agent's status to produce a stable policy...................
